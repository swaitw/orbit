...
<ReactExample name="InputField help" />

#### Error message

<ReactExample name="InputField error" />
...
That's why our native fields come with a <InlineToken value="paletteCloudNormal" /> background
to stand out against the <InlineToken value="paletteWhite" /> background natural to the app.
And they don't have a border as that would make them too heavy and overwhelming.

For our desktop and responsive fields,
which are usually placed against a <InlineToken value="paletteCloudLight" /> background,
the fields have a <InlineToken value="paletteWhite" /> background to stand out.
...
<ReactExample name="InputField suffixes" />
